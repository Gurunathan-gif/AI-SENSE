...
export function generateExactOrSynthesizedCode(prompt) {
  ...
  // Clean prompt by removing common natural language phrases
...
void loop() {
  ...
  // Map pulse frequencies to 0-255 RGB values
...
void loop() {
  ...
  if (fingerprintID > 0) {
    ...
    digitalWrite(RELAY_PIN, HIGH); // Unlock door for 3 seconds
    delay(3000);
    digitalWrite(RELAY_PIN, LOW);
...
int getFingerprintID() {
  ...
  // 3. HX711 Load Cell Weight Scale
    ...
    componentsNeeded.push("HX711 Amplifier Module", "5kg / 10kg Strain Gauge Load Cell", "LCD 16x2 Display");
    wiring = [
      ...
      "HX711 GND -> GND",
      "HX711 DT -> Digital D2",
      "HX711 SCK -> Digital D3"
    ];
    code = `/*
 * AI SENSE — HX711 Load Cell Weight Measurement
 * High Precision Strain Gauge Amplifier
 */
...
HX711 scale;
float calibration_factor = -7050; // Adjust for your load cell calibration
...
void loop() {
  if (scale.is_ready()) {
    float weightGrams = scale.get_units(5) * 1000.0; // Read 5 average samples
    Serial.print("TELEMETRY|WEIGHT_GRAMS:");
    Serial.print(weightGrams, 1);
    Serial.println("G");
  } else {
    Serial.println("TELEMETRY|STATUS:HX711_NOT_READY");
  }
  delay(500);
}`;
  }
...
void loop() {
  ...
  else if (cleanedPrompt.includes("dht") || cleanedPrompt.includes("dht11") || cleanedPrompt.includes("dht22") || (cleanedPrompt.includes("temperature") && cleanedPrompt.includes("humidity"))) {
...
void loop() {
  ...
  // 6. MQ-2 / MQ-135 Gas & Smoke Sensor
  else if (cleanedPrompt.includes("mq") || cleanedPrompt.includes("mq-2") || cleanedPrompt.includes("gas") || cleanedPrompt.includes("smoke")) {
    title = "MQ Gas & Smoke Leak Detection Alarm";
    componentsNeeded.push("MQ Gas Sensor", "Piezo Alarm Buzzer", "Status Red LED");
    wiring = ["MQ VCC -> 5V", "MQ GND -> GND", "MQ AOUT -> Analog A0", "Buzzer -> Digital D9"];
    code = `/*
...
void loop() {
  ...
  if (gasAdc > 350) tone(BUZZER_PIN, 2500);
  ...
  else if (cleanedPrompt.includes("pir") || cleanedPrompt.includes("motion")) {
    title = "PIR Motion Security Detector";
    componentsNeeded.push("HC-SR501 PIR Motion Sensor", "Buzzer", "LED");
    wiring = ["PIR VCC -> 5V", "PIR GND -> GND", "PIR OUT -> Digital D3", "Buzzer -> D8", "LED -> D13"];
    code = `/*
 * AI SENSE — PIR Motion Security Detector
 */
...
void loop() {
  ...
  // 8. Universal Heuristic Synthesizer for any other unlisted custom hardware
  else {
